...
void OdomNode::imu_cb(const sensor_msgs::Imu::ConstPtr& msg) {
    ...
    if(OdomNode::imu_cb_count == 5) {
    ...
    else if(OdomNode::imu_cb_count > 100 && (OdomNode::imu_cb_count < linear_offset_array.size()+100)) {
        ...
        OdomNode::linear_offset_array[OdomNode::linear_offset_array.size()-1][0] = msg->linear_acceleration.x;
        OdomNode::linear_offset_array[OdomNode::linear_offset_array.size()-1][1] = msg->linear_acceleration.y;
        OdomNode::linear_offset_array[OdomNode::linear_offset_array.size()-1][2] = msg->linear_acceleration.z;

        OdomNode::angular_offset_array[OdomNode::linear_offset_array.size()-1][0] = msg->angular_velocity.x;
        OdomNode::angular_offset_array[OdomNode::linear_offset_array.size()-1][1] = msg->angular_velocity.y;
        OdomNode::angular_offset_array[OdomNode::linear_offset_array.size()-1][2] = msg->angular_velocity.z;
...
void OdomNode::update_position() {
    ...
    delta_a[0] = OdomNode::imu_new.linear_acceleration.x - OdomNode::imu_old.linear_acceleration.x - OdomNode::linear_offset[0];
    delta_a[1] = OdomNode::imu_new.linear_acceleration.y - OdomNode::imu_old.linear_acceleration.x - OdomNode::linear_offset[1];
    delta_a[2] = OdomNode::imu_new.linear_acceleration.z - OdomNode::imu_old.linear_acceleration.x - OdomNode::linear_offset[2];
    ...
    linear_vel_tmp[0] = (OdomNode::imu_old.linear_acceleration.x-OdomNode::linear_offset[0])*delta_t;
    linear_vel_tmp[1] = (OdomNode::imu_old.linear_acceleration.y-OdomNode::linear_offset[1])*delta_t;
    linear_vel_tmp[2] = (OdomNode::imu_old.linear_acceleration.z-OdomNode::linear_offset[2])*delta_t;

    linear_pos_tmp[0] =  0.5*delta_a[0]*delta_t*delta_t + OdomNode::linear_velocity[0] * delta_t;
    linear_pos_tmp[1] =  0.5*delta_a[1]*delta_t*delta_t + OdomNode::linear_velocity[1] * delta_t;
    linear_pos_tmp[2] =  0.5*delta_a[2]*delta_t*delta_t + OdomNode::linear_velocity[2] * delta_t;
    ...
    angular_pos_tmp[0] = (OdomNode::imu_old.angular_velocity.x - OdomNode::angular_offset[0]) * delta_t;
    angular_pos_tmp[1] = (OdomNode::imu_old.angular_velocity.y - OdomNode::angular_offset[1]) * delta_t;
    angular_pos_tmp[2] = (OdomNode::imu_old.angular_velocity.z - OdomNode::angular_offset[2]) * delta_t;
    ...
    if ((fabs(linear_pos_tmp[0]) > 0.005 || fabs(linear_pos_tmp[1]) > 0.005) &&
        (fabs(linear_pos_tmp[0]) < 0.5 || fabs(linear_pos_tmp[1]) < 0.5) &&
        (delta_t<0.2) &&
